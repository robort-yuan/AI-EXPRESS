...
bool WebsocketConfig::CheckConfig() {
  ...
  if (json_.isMember("chnn_list")) {
    const auto chnn_list_obj = json_["chnn_list"];
    if (!chnn_list_obj.isNull()) {
      int chnn_size = chnn_list_obj.size();
      for (int i = 0; i < chnn_size; ++i) {
        const Json::Value one_ch_obj = chnn_list_obj[i];
        chnn_list_.push_back(one_ch_obj.asInt());
      }
    }
  }
  return true;
