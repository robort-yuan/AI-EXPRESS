...
/**
 *  @brief Function get_bits()  读取N个bit, N 不能超过32
 *  @param[in]     h     get_bit_context structrue
...
static int get_bits(void *h, int n) {
  ...
  if (n == MAX_LEN) {
    ...
    RPT(RPT_DBG, "0x%x 0x%x 0x%x 0x%x", (*cur_char), *(cur_char + 1),
        *(cur_char + 2), *(cur_char + 3));
  }
...
static int parse_codenum(void *buf) {
  ...
  code_num =
      ((uint32_t)1 << leading_zero_bits) - 1 + get_bits(buf, leading_zero_bits);

  return code_num;
}

/**
 *  @brief Function parse_ue()
 *  指数哥伦布编码解析 ue(), 参考h264标准第九节
 *  @param[in]     buf       sps_pps parse buf
 *  @retval        code_num
 *  @pre
 *  @post
 */
static int parse_ue(void *buf) { return parse_codenum(buf); }

/**
 *  @brief Function parse_se() ָ
 *  指数哥伦布编码解析 se(), 参考h264标准第九节
 *  @param[in]     buf       sps_pps parse buf
 *  @retval        code_num
 *  @pre
 *  @post
 ...
 *      H.264编码时，在每个NAL前添加起始码 0x000001,
 * 解码器在码流总检测到起始码，当前NAL结束
 *      为了防止NAL内部出现0x000001的数据，h.264又提出'防止竞争emulation
 * prevention'机制
 *      在编码温暖一个NAL时，如果检测出有连续两个0x00字节，就在后面插入一个0x03。
 *      当解码器在NAL内部检测到0x000003的数据，就把0x03抛弃，回复原始数据。
 *      0x000000  >>>>>>  0x00000300
 *      0x000001  >>>>>>  0x00000301
 *      0x000002  >>>>>>  0x00000302
 *      0x000003  >>>>>>  0x00000303
...
/**
 *  @brief Function get_bit_context_free()
 *  VUI_parameters 解析，原来参考h264标准
 *  @param[in]     buf       get_bit_context buf
...
/**
 *  @brief Function h264dec_seq_parameter_set()  h264 SPS infomation解析
...
/**
 *  @brief Function more_rbsp_data()
 * 计算pps串最后一个为1的比特位及其后都是比特0的个数
...
static int more_rbsp_data(void *buf) {
  ...
  for (tmp.bit_pos = ptr->total_bit - 1; tmp.bit_pos > ptr->bit_pos;
       tmp.bit_pos -= 2) {
    if (get_1bit(&tmp)) {
      break;
    }
 ...
 *  @param[in]     pps_ptr   ppsָ指针，保存pps信息
 *  @retval        0: success, -1 : failure
 *  @pre
 *  @post
 */
    ...
    if (pps->slice_group_map_type == 0) {
      ...
      for (i = 0; i < pps->pic_size_in_map_units_minus1; i++) {
        ...
        pps->slice_group_id[i] =
            get_bits(buf, pps->pic_size_in_map_units_minus1);
      }
    }
  }
          ...
          if (i < 6) {
            ...
            for (j = 0; j < 64; j++) {
              if (next_scale != 0) {
                delta_scale = parse_se(buf);
                next_scale = (last_scale + delta_scale + 256) % 256;
                pps->UseDefaultScalingMatrix8x8Flag[ii] =
                    ((j == 0) && (next_scale == 0));
              }
              pps->ScalingList8x8[ii][j] =
                  (next_scale == 0) ? last_scale : next_scale;
...
int h264_get_height(SPS *sps_ptr) {
  return (sps_ptr->pic_height_in_map_units_minus1 + 1) * 16 *
         (2 - sps_ptr->frame_mbs_only_flag);
}

int h264_get_format(SPS *sps_ptr) { return sps_ptr->frame_mbs_only_flag; }
...
int h264_get_framerate(float *framerate, SPS *sps_ptr) {
  ...
  if (sps_ptr->vui_parameters.timing_info_present_flag) {
    if (sps_ptr->frame_mbs_only_flag) {
      *framerate = (float)sps_ptr->vui_parameters.time_scale /
                   (float)sps_ptr->vui_parameters.num_units_in_tick;
      // fr_int = sps_ptr->vui_parameters.time_scale /
      // sps_ptr->vui_parameters.num_units_in_tick;
    } else {
      *framerate = (float)sps_ptr->vui_parameters.time_scale /
                   (float)sps_ptr->vui_parameters.num_units_in_tick / 2.0;
      // fr_int = sps_ptr->vui_parameters.time_scale /
      // sps_ptr->vui_parameters.num_units_in_tick / 2;
    }
    fr_int = sps_ptr->vui_parameters.time_scale /
             sps_ptr->vui_parameters.num_units_in_tick / 2;
  }
